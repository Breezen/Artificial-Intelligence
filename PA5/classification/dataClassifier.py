...
def enhancedFeatureExtractorDigit(datum):
    ...
    - numDiffX: the number of (black->white) or (white->black) in column x
    - numDiffY: the number of (black->white) or (white->black) in row y
    - numWhiteArea: the number of contiguous region of white space
    I use duplicated features to adjust the weight of one feature among all
    ...
    def numDiffX(pixels):
        ...
        for x in xrange(DIGIT_DATUM_WIDTH):
            n = 0
            for y in xrange(DIGIT_DATUM_HEIGHT - 1):
                if (pixels[x][y] == 0 and pixels[x][y + 1] != 0) or (pixels[x][y] != 0 and pixels[x][y + 1] == 0):
                    n += 1
            for i in xrange(8):
                for k in xrange(2):
                    newFeatures["numDiffX(%d)(%d)%d" % (x, i, k)] = 1 if n == i else 0
        return newFeatures
    ...
    def numDiffY(pixels):
        ...
        for y in xrange(DIGIT_DATUM_HEIGHT):
            n = 0
            for x in xrange(DIGIT_DATUM_WIDTH - 1):
                if (pixels[x][y] == 0 and pixels[x + 1][y] != 0) or (pixels[x][y] != 0 and pixels[x + 1][y] == 0):
                    n += 1
            for i in xrange(8):
                for k in xrange(2):
                    newFeatures["numDiffY(%d)(%d)%d" % (y, i, k)] = 1 if n == i else 0
        return newFeatures

    def numWhiteArea(pixels):
        n = 0
        for x in xrange(DIGIT_DATUM_WIDTH):
            for y in xrange(DIGIT_DATUM_HEIGHT):
                if pixels[x][y] == 0:
        ...
        for i in xrange(6):
            for k in xrange(19):
                newFeatures["numWhiteArea(%d)%d" % (i, k)] = 1 if n == i else 0
        return newFeatures

    def BFS(pixels, x0, y0):
        q = util.Queue()
        q.push((x0, y0))
        pixels[x0][y0] = 2
        while not q.isEmpty():
            x, y = q.pop()
            if x > 0 and pixels[x - 1][y] == 0:
                q.push((x - 1, y))
                pixels[x - 1][y] = 2
            if x < DIGIT_DATUM_WIDTH - 1 and pixels[x + 1][y] == 0:
                q.push((x + 1, y))
                pixels[x + 1][y] = 2
            if y > 0 and pixels[x][y - 1] == 0:
                q.push((x, y - 1))
                pixels[x][y - 1] = 2
            if y < DIGIT_DATUM_HEIGHT - 1 and pixels[x][y + 1] == 0:
                q.push((x, y + 1))
                pixels[x][y + 1] = 2

    pixels = datum.getPixels()
    pixelsCopy = [list(column) for column in pixels]
    features += numWhiteArea(pixelsCopy)
    features += numDiffX(pixels)
    features += numDiffY(pixels)
...
def enhancedPacmanFeatures(state, action):
    ...
    ghostPosns = nextState.getGhostPositions()
    ghostDis = float("inf") if len(ghostPosns) == 0 \
    else min([util.manhattanDistance(pacmanPos, ghostPos) for ghostPos in ghostPosns])

    foodPosns = nextState.getFood().asList()
    foodDis = float("inf") if len(foodPosns) == 0 \
    else min([util.manhattanDistance(pacmanPos, foodPos) for foodPos in foodPosns])

    features["ghostDis"] = 10.0 / ghostDis if ghostDis else 0
    features["foodDis"] = 4.0 / foodDis if foodDis else 0
    features["numFood"] = nextState.getNumFood()
    features["scoreChange"] = nextState.getScore() - state.getScore()
...
def analysis(classifier, guesses, testLabels, testData, rawTestData, printImage):
    ...
    # countX: (3.8711743772241993, 0, 16)
    # countY: (6.48984771573604, 0, 20)

    # total, n, mini, maxi = 0.0, 0.0, float("inf"), float("-inf")
    # for i in range(len(guesses)):
    #     pixels = rawTestData[i].getPixels()
    #     for x in xrange(DIGIT_DATUM_WIDTH):
    #         data = 0
    #         for y in xrange(DIGIT_DATUM_HEIGHT):
    #             if pixels[x][y]: data += 1
    #         if data > 0: n += 1
    #         total += data
    #         mini = min(mini, data)
    #         maxi = max(maxi, data)
    # print (total / n, mini, maxi)
