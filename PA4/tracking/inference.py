...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        n, posns = self.numParticles, self.legalPositions
        particles, count = [], 0
        while count < n:
            for pos in posns:
    ...
    def observe(self, observation, gameState):
        ...
        for pos in self.legalPositions:
            ...
            allPossible[pos] += oldBeliefs[pos] * emissionModel[trueDistance]
        allPossible.normalize()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        n, nGhost, posns = self.numParticles, self.numGhosts, self.legalPositions
        ghostPosns = list(itertools.product(posns, repeat = nGhost))
        random.shuffle(ghostPosns)

        particles, count = [], 0
        while count < n:
            for pos in ghostPosns:
                particles.append(pos)
                count += 1
                if count == n: break
        self.particles = particles
        ...
        for particle in self.particles:
            ...
            for i in xrange(self.numGhosts):
                ...
                if noisyDistances[i] == None:
                    ...
                    p *= emissionModels[i][trueDistance]
